...
export default function CustomerServiceReminderPage() {
  const query = api.admin.listServiceReminders.useQuery({ take: 100 }, { retry: false });
        ...
        <Table>
          ...
          <TableBody>
            {(query.data?.items ?? []).map((r) => {
              const customer = r.workOrder.customer;
              const vehicle = r.workOrder.vehicle;
              const vehicleLabel = vehicle
                ? `${vehicle.plateNumber} - ${vehicle.brand ?? ""} ${vehicle.model ?? ""}`.trim()
                : "-";

              return (
                <TableRow key={r.id}>
                  <TableCell>{formatDateTime(r.dueDate)}</TableCell>
                  <TableCell className="font-semibold">{customer?.name ?? "-"}</TableCell>
                  <TableCell>{customer?.phone ?? "-"}</TableCell>
                  <TableCell>{vehicleLabel}</TableCell>
                  <TableCell className="font-semibold">{r.workOrder.woNumber}</TableCell>
                  <TableCell>
                    <span
                      className={cn(
